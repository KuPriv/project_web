...
function saveLocalStorage(values) {
  ...
  localStorage.setItem(
    "message",
    values.message === null ? "" : values.message
  );
      ...
      <div>
        <Formik
          ...
          onSubmit={(values, { setSubmitting }) => {
            ...
            const prom = fetch("https://formcarry.com/s/13RVRgb18", {
              method: "POST",
              mode: "cors",
              cache: "no-cache",
            ...
            prom.then((response) => {
              if (response.ok) this.props.submitSuccess();
              else this.props.submitError();
              setSubmitting(false);
            });
          }}
        >
...
class ModalWindow extends React.Component {
  ...
  stepOpen(timestamp) {
    ...
    if (this.id) {
      ...
      document.getElementById("moving-overlay").style.transformOrigin =
        centerString;
    }
        ...
        <Route path="/form">
          <div id="my-fixed-overlay">
            <div id="moving-overlay" onClick={this.handleOffClick}>
              <div id="my-modal">{this.props.children}</div>
            </div>
          </div>
        </Route>
      </Switch>
...
const WrappedModalWindow = connect(
  mapStateOpener,
  mapDispatchOpener
)(withRouter(ModalWindow));
